(function (global, factory) {
	...
	class DateTimeValue extends AbstractValue {
	  ...
	  static fromData(data, timezone) {
	    const icalValue = ICAL__default["default"].Time.fromData(data, timezone ? timezone.toICALTimezone() : undefined);
	    return DateTimeValue.fromICALJs(icalValue);
	  }
	...
	class RecurrenceManager {
	  ...
	  *getRecurrenceExceptionIterator() {
	  ...
	  getRangeRecurrenceExceptionForId(recurrenceId) {
	    ...
	    const index = ICAL__default["default"].helpers.binsearchInsert(this._rangeRecurrenceExceptionItemsIndex, recurrenceId, (a, b) => a - b);
	  ...
	  relateRecurrenceException(recurrenceExceptionItem) {
	    ...
	    if (recurrenceExceptionItem.modifiesFuture()) {
	      ...
	      const index = ICAL__default["default"].helpers.binsearchInsert(this._rangeRecurrenceExceptionItemsIndex, key, (a, b) => a - b);
	  ...
	  *getAllOccurrencesBetweenIterator(queriedTimeRangeStart, queriedTimeRangeEnd) {
	  ...
	  _getRecurExpansionObject() {
	    ...
	    for (let rDateValue of this.getRecurrenceDateIterator()) {
	      ...
	      const index = ICAL__default["default"].helpers.binsearchInsert(ruleDates, rDateValue, (a, b) => a.compare(b));
	    ...
	    if (ruleDates.length > 0 && ruleDates[0].compare(dtstart) === -1) {
	      ...
	      ruleDateInc = ICAL__default["default"].helpers.binsearchInsert(ruleDates, dtstart, (a, b) => a.compare(b));
	      ruleDate = exDates[ruleDateInc];
	    }

	    for (let exDateValue of this.getRecurrenceDateIterator(true)) {
	      exDateValue = exDateValue.toICALJs();
	      const index = ICAL__default["default"].helpers.binsearchInsert(exDates, exDateValue, (a, b) => a.compare(b));
	      exDates.splice(index, 0, exDateValue);
	    }

	    const exDateInc = ICAL__default["default"].helpers.binsearchInsert(exDates, dtstart, (a, b) => a.compare(b));
	...
	class Timezone {
	  ...
	  constructor(timezoneId, ics) {
	    ...
	    if (timezoneId instanceof ICAL__default["default"].Timezone) {
	      this._innerValue = timezoneId;
	      this._initialized = true;
	    } else if (timezoneId instanceof ICAL__default["default"].Component) {
	      this._innerValue = new ICAL__default["default"].Timezone(timezoneId);
	      this._initialized = true;
	    } else {
	  ...
	  offsetForArray(year, month, day, hour, minute, second) {
	    ...
	    const time = new ICAL__default["default"].Time({
	      year,
	      month,
	      day,
	      hour,
	  ...
	  timestampToArray(ms) {
	    ...
	    const time = ICAL__default["default"].Time.fromData({
	      year: 1970,
	      month: 1,
	      day: 1,
	      hour: 0,
	  ...
	  _initialize() {
	    if (!this._initialized) {
	      const jCal = ICAL__default["default"].parse(this._ics);
	      const icalComp = new ICAL__default["default"].Component(jCal);
	      this._innerValue = new ICAL__default["default"].Timezone(icalComp);
	      this._initialized = true;
	    }
	  }

	}
	Timezone.utc = new Timezone(ICAL__default["default"].Timezone.utcTimezone);
	Timezone.floating = new Timezone(ICAL__default["default"].Timezone.localTimezone);
	...
	if (!(ICAL__default["default"].TimezoneService instanceof TimezoneAdapter)) {
	  ICAL__default["default"].TimezoneService = new TimezoneAdapter(getTimezoneManager());
	}
	/**
